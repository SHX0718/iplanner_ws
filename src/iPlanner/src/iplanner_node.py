...
class iPlannerNode:
    ...
    def pubImage(self):
        ...
        try:
            ...
            img_normalized = self.img.copy()
            img_max = np.max(img_normalized[img_normalized > 0])
            if img_max > 0:
                img_normalized = img_normalized / img_max
            else:
                img_normalized = img_normalized
            
            # Convert to uint8 for ROS Image message
            img_vis = (img_normalized * 255).astype(np.uint8)
            
            # Convert numpy array to ROS Image message
            img_msg = ros_numpy.numpify(self.img)
            img_msg = ros_numpy.msgify(Image, img_vis, encoding='mono8')
